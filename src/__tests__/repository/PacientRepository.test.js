...
//aqui definimos o comportamental antes, para cada teste e depois, usando as func definidas em database/setup.js
beforeAll(async () => {
    await db.setUp();
});
...
//em suma, esses testes são apenas p os possíveis cenarios de 'criar novo paciente'

//neles o 'savePacient' real da nossa aplicação tenta salvar dados no db em memória q criamos e observa os retornos
describe('Pacient Repository Test', () => {
    ...
    it('Should receive error when missing birth date', async () => {
        ...
        expect(async () => {
            await PacientRepository.savePacient(pacient);
        }).rejects.toThrow(new Error("ValidationError: email: E-mail contact is required."));
    ...
    it('Should validate phone format', async () => {
        ...
        expect(async () => {
            await PacientRepository.savePacient(pacient);
        }).rejects.toThrow(new Error("ValidationError: phone: 11988772201 This is not a valid phone value. Please use the following format 99 91234-5678"));
